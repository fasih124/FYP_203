...
int micInterval = 500; //5sec in mic sensor code
...
//Mic sensor execution block
    #pragma region 
    if(Firebase.ready() && signupCheck && (millis() - prevTimeMicSentData > micInterval || prevTimeMicSentData == 0))
    {
      prevTimeMicSentData=millis();  //update sent time
      
      //String type diaper condition
      if(Firebase.RTDB.setBool(&fbdo, "sensors/Crying", detect_Cry()))
      {
        //raw value
        Firebase.RTDB.setInt(&fbdo, "ignoreValues/Mic", mic_Raw_Value());

        Serial.println("Cry status pushed!");
        Serial.print("Path: "); Serial.println(fbdo.dataPath());
        Serial.print("Type: "); Serial.println(fbdo.dataType());
